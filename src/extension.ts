...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 初始化 workspace indexer（必须在 chatViewProvider 之前）
    ...
    chatViewProvider = new ChatViewProvider(
        context.extensionUri, client, fileSystem, modeManager, decorationManager, context, workspaceIndexer
    );
    ...
    toolServer.start().then(port => {
        ...
        client.registerTools().then(success => {
            if (success) {
                outputChannel.appendLine(`[Extension] Tools registered with Dify`);
            } else {
                outputChannel.appendLine(`[Extension] Tool registration skipped (Dify manages tools internally)`);
            }
        });
    }).catch(err => {
        outputChannel.appendLine(`[Extension] Tool server failed: ${err.message}`);
    });
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('dify.openChat', () => {
            checkConfigAndRun(() => vscode.commands.executeCommand('difyChatView.focus'));
        })
    );
        ...
        vscode.commands.registerCommand('dify.queryKnowledge', async () => {
            ...
            try {
                ...
                const panel = vscode.window.createWebviewPanel(
                    'knowledgeResult', '知识库查询结果',
                    vscode.ViewColumn.Beside, { enableScripts: true }
                );
                panel.webview.html = knowledgeResultHtml(query, result.answer);
            } catch (error: any) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('dify.queryArchitecture', async () => {
            try {
                ...
                panel.webview.html = knowledgeResultHtml('项目架构', result);
            } catch (error: any) {
                vscode.window.showErrorMessage(`查询失败: ${error.message}`);
            }
        })
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('dify.queryBestPractices', async () => {
            const topic = await vscode.window.showInputBox({
                prompt: '输入主题', placeHolder: '例如: 代码规范、设计模式'
            });
            if (!topic) return;
            try {
                ...
                const panel = vscode.window.createWebviewPanel(
                    'bestPracticesResult', '最佳实践',
                    vscode.ViewColumn.Beside, { enableScripts: true }
                );
                panel.webview.html = knowledgeResultHtml(`最佳实践: ${topic}`, result);
            } catch (error: any) {
                vscode.window.showErrorMessage(`查询失败: ${error.message}`);
            }
        })
